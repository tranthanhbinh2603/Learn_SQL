...
-------------------------------------------------------------------------------------------------------
--Bài học
--1. Truy vấn toàn bộ dữ liệu trong 1 bảng
SELECT * FROM dbo.BOMON
SELECT * FROM dbo.CHUDE
SELECT * FROM dbo.BOMON, dbo.CHUDE --Nhận xét: Sản sinh ra số test tương ứng theo tích đề các: m x n (m là số trường ở trên, n là số trường ở dưới)
--2. Truy vấn 1 hoặc nhiều trường trong 1 bảng
SELECT TENBM FROM dbo.BOMON
SELECT dbo.BOMON.MABM, dbo.BOMON.TENBM FROM dbo.BOMON
--3. Truy vấn ra các kết quả không trùng nhau
SELECT MAKHOA FROM dbo.BOMON
SELECT DISTINCT MAKHOA FROM dbo.BOMON
--Tiện thể ta có ứng dụng đếm số trường
SELECT COUNT(MAKHOA) AS N'Số trường' FROM dbo.BOMON 
SELECT COUNT(DISTINCT MAKHOA) AS N'Số trường' FROM dbo.BOMON 
--Đặt tên cho cột chưa đặt tên: As N'<tên cột>. Chú ý là trước From
--4. Tìm kiếm 1 số lượng % nhất định
SELECT * FROM dbo.BOMON --Tất cả
SELECT TOP 5 * FROM dbo.BOMON --5 cái đầu
SELECT TOP 5 PERCENT * FROM dbo.BOMON --5% đầu tiên
--5. Truy vấn có điều kiện: 
--Lấy ra Mã GV, tên GV và họ tên người thân tương ứng
SELECT dbo.GIAOVIEN.MAGV, dbo.GIAOVIEN.HOTEN, dbo.NGUOITHAN.TEN FROM dbo.GIAOVIEN, dbo.NGUOITHAN WHERE dbo.GIAOVIEN.MAGV = dbo.NGUOITHAN.MAGV
...
--Xuất ra giáo viên có lương hơn 2000
...
--Lấy ra những giáo viên lớn hơn 40 tuổi. 
...
SELECT gv1.HOTEN, gv2.HOTEN FROM dbo.GIAOVIEN AS gv1, dbo.GIAOVIEN AS gv2 WHERE gv1.MAGV = gv2.GVQLCM
--2. Xuất ra số lượng giáo viên của khoa CNTT
SELECT Gv.* FROM dbo.GIAOVIEN AS GV, dbo.THAMGIADT AS DT WHERE GV.MAGV = DT.MAGV AND DT.KETQUA = N'Đạt'
